...
// const containers = ['class', 'module', 'external', 'namespace', 'mixin', 'interface', 'service', 'component', 'delegate', 'reducer', 'route', 'util'];
...
function buildLink(longname, linkText, options) {
    ...
    // (but skip anything that looks like an inline tag or HTML tag)
        ...
        stringifiedType = stringifyType(parsedType, options.cssClass, options.linkMap);
    ...
    text = !options.outputAsJSON && options.monospace ? `<code>${text}</code>` : text;
    ...
    if (!options.outputAsJSON) {
        ...
        // dump the entire thing back to the caller and let them pick what they need:
        return {
            hasURI: !!fileUrl,
            uri: fileUrl ? encodeURI(env.conf.templates.baseUrl + fileUrl + fragmentString) : null,
            class: options.cssClass,
            text: text,
            fragmentString: fragmentString,
            fragmentId: options.fragmentId,
            longname: longname,
            strippedLongName: stripped,
            linkText: linkText,
            isComplexTypeExpression: isComplexTypeExpression(longname),
 ...
 * @param {options=} options set `options.outputAsJSON` boolean to instruct the function
 * to produce a *JSON object* rather than a *string* for the given link.
 * @return {string|object} The HTML link, or a plain-text string if the link is not available.
 */
const linkto = exports.linkto = (longname, linkText, cssClass, fragmentId, options) => buildLink(longname, linkText, {
    cssClass: cssClass,
    fragmentId: fragmentId,
    linkMap: longnameToUrl,
    outputAsJSON: options && options.outputAsJSON
});
 ...
 * options.
 */
const toTutorial = exports.toTutorial = (tutorial, content, missingOpts, options) => {
    let classname;
    let link;
    let node;
    let tag;
    ...
    if (!options.outputAsJSON) {
        ...
        return {
            ...
            missingOpts: missingOpts,
            options: options,
            tag: missingOpts ? missingOpts.tag : null,
            classname: missingOpts ? missingOpts.classname : null,
            node: node
        };
    }
};
 ...
 */
exports.resolveLinks = (str, options) => {
    options = options || {};
    let replacers;
    ...
    if (!options.outputAsJSON) {
        ...
        var rv = inline.replaceInlineTags(str, replacers);

        rv.source = str;
        // When we didn't transform anything, we produce a simpler output:
        if (rv.newString === str && rv.tags.length === 0) {
            return str;
        }
 ...
 */
exports.getAncestorLinks = (data, doclet, cssClass, options) => {
    ...
    ancestors.forEach(ancestor => {
        ...
        const link = linkto(ancestor.longname, linkText, cssClass, null, options);
    ...
    if (links.length) {
        if (!options.outputAsJSON) {
            ...
            links[links.length - 1].scopePunctuation = (exports.scopeToPunc[doclet.scope] || '');
        }
    }
 ...
 *
 * unless the optional `options` tell me otherwise.
 *
 * @param {TAFFY} data The TaffyDB database to prune.
 * @param {options} options options:
 *                  - `keepUndocumented`
 *                  - `keepMarkedAsIgnore`
 *                  - `keepMembersOfAnonymous`
 *                  - `keepAllAccessLevels`
 *                  - `keepEveryone`
 *
 * @return {TAFFY} The pruned database.
 */
exports.prune = (data, options) => {
    options = options || {};
    if (!options.keepUndocumented && !options.keepEveryone) {
        data({undocumented: true}).remove();
    }
    if (!options.keepMarkedAsIgnore && !options.keepEveryone) {
        data({ignore: true}).remove();
    }
    if (!options.keepMembersOfAnonymous && !options.keepEveryone) {
        data({memberof: '<anonymous>'}).remove();
    } else {
        data({memberof: '<anonymous>'}).update({ isMemberOfAnonymous: true });
    }

    if (!options.keepAllAccessLevels && !options.keepEveryone) {
...
exports.createLink = doclet => {
    ...
    let match;
    let isContainer = containerHasKind(doclet.kind);
