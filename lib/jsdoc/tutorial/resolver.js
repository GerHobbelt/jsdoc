...
function addTutorialConf(name, meta) {
  ...
  if (isTutorialJSON(meta)) {
    ...
    if (hasOwnProp.call(conf, name)) {
      logger.warn(
        'Metadata for the tutorial %s is defined more than once. Only the first definition will be used.',
        name,
      );
    } else {
      conf[name] = meta;
...
exports.addTutorial = function(current) {
  if (exports.root.getByName(current.name)) {
    logger.warn(
      'The tutorial %s is defined more than once. Only the first definition will be used.',
      current.name,
    );
  } else {
