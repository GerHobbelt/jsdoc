...
var regurgitate = exports.regurgitate = function(taffyData, opts, tutorials) {
    ...
    // set up templating
    view.layout = conf.wiki.layoutFile ?
        path.getResourcePath(path.dirname(conf.wiki.layoutFile),
            path.basename(conf.wiki.layoutFile) ) :
        'layout.tmpl';
    ...
    if (0) {
        ...
        if (conf.wiki.staticFiles) {
            ...
            staticFilePaths = conf.wiki.staticFiles.include ||
                conf.wiki.staticFiles.paths ||
                [];
            staticFileFilter = new (require('jsdoc/src/filter')).Filter(conf.wiki.staticFiles);
